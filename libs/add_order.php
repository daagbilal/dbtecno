...
if (isLoggedIn()) {
    ...
    if (isset($_POST["order_submit"])) {
        ...
        FROM (
            SELECT computers.urun_kodu, computers.fiyat, sepet.miktar
            FROM computers
            INNER JOIN sepet ON computers.urun_kodu = sepet.urun_kodu
            WHERE musteri_id = ?
            UNION ALL
            SELECT phones.urun_kodu, phones.fiyat, sepet.miktar
            FROM phones
            INNER JOIN sepet ON phones.urun_kodu = sepet.urun_kodu
            WHERE musteri_id = ?
            UNION ALL
            SELECT televisions.urun_kodu, televisions.fiyat, sepet.miktar
            FROM televisions
            INNER JOIN sepet ON televisions.urun_kodu = sepet.urun_kodu
            WHERE musteri_id = ?
            UNION ALL
            SELECT tablets.urun_kodu, tablets.fiyat, sepet.miktar
            FROM tablets
            INNER JOIN sepet ON tablets.urun_kodu = sepet.urun_kodu
            WHERE musteri_id = ?
            UNION ALL
            SELECT smart_watchs.urun_kodu, smart_watchs.fiyat, sepet.miktar
            FROM smart_watchs
