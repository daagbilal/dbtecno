...
if (isLoggedIn()) {
    ...
    FROM (
        SELECT computers.urun_kodu, computers.resim_adi, computers.marka, computers.model, computers.seri, computers.fiyat, orders.miktar, orders.durum, orders.siparis_id, orders.add_time
        FROM computers
        INNER JOIN orders ON computers.urun_kodu = orders.urun_kodu
        WHERE musteri_id = ?
        UNION ALL
        SELECT phones.urun_kodu, phones.resim_adi, phones.marka, phones.model, phones.seri, phones.fiyat, orders.miktar, orders.durum, orders.siparis_id, orders.add_time
        FROM phones
        INNER JOIN orders ON phones.urun_kodu = orders.urun_kodu
        WHERE musteri_id = ?
        UNION ALL
        SELECT televisions.urun_kodu, televisions.resim_adi, televisions.marka, televisions.model, televisions.seri, televisions.fiyat, orders.miktar, orders.durum, orders.siparis_id, orders.add_time
        FROM televisions
        INNER JOIN orders ON televisions.urun_kodu = orders.urun_kodu
        WHERE musteri_id = ?
        UNION ALL
        SELECT tablets.urun_kodu, tablets.resim_adi, tablets.marka, tablets.model, tablets.seri, tablets.fiyat, orders.miktar, orders.durum, orders.siparis_id, orders.add_time
        FROM tablets
        INNER JOIN orders ON tablets.urun_kodu = orders.urun_kodu
        WHERE musteri_id = ?
        UNION ALL
        SELECT smart_watchs.urun_kodu, smart_watchs.resim_adi, smart_watchs.marka, smart_watchs.model, smart_watchs.seri, smart_watchs.fiyat, orders.miktar, orders.durum, orders.siparis_id, orders.add_time
        FROM smart_watchs
