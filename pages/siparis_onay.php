...
if (isLoggedIn()) {
    ...
    FROM (
        SELECT computers.urun_kodu, computers.resim_adi, computers.marka, computers.model, computers.seri, computers.fiyat, sepet.miktar
        FROM computers
        INNER JOIN sepet ON computers.urun_kodu = sepet.urun_kodu
        WHERE musteri_id = ?
        UNION ALL
        SELECT phones.urun_kodu, phones.resim_adi, phones.marka, phones.model, phones.seri, phones.fiyat, sepet.miktar
        FROM phones
        INNER JOIN sepet ON phones.urun_kodu = sepet.urun_kodu
        WHERE musteri_id = ?
        UNION ALL
        SELECT televisions.urun_kodu, televisions.resim_adi, televisions.marka, televisions.model, televisions.seri, televisions.fiyat, sepet.miktar
        FROM televisions
        INNER JOIN sepet ON televisions.urun_kodu = sepet.urun_kodu
        WHERE musteri_id = ?
        UNION ALL
        SELECT tablets.urun_kodu, tablets.resim_adi, tablets.marka, tablets.model, tablets.seri, tablets.fiyat, sepet.miktar
        FROM tablets
        INNER JOIN sepet ON tablets.urun_kodu = sepet.urun_kodu
        WHERE musteri_id = ?
        UNION ALL
        SELECT smart_watchs.urun_kodu, smart_watchs.resim_adi, smart_watchs.marka, smart_watchs.model, smart_watchs.seri, smart_watchs.fiyat, sepet.miktar
        FROM smart_watchs
